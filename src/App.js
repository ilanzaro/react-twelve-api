...
function App() {
  ...
  useEffect(() => {
    document.body.dir =
      LANGUAGES.find((l) => l.code === navigator.language).dir || "ltr";
    ...
    return (
      <Link to={to}>
        <span className={isLogo ? "nav_logo" : "nav_link"}>
          ...
          {isExpended ? (
            <span className={isLogo ? "nav_logo-name" : "nav_name"}>
              {name}
            </span>
          ) : (
            ""
    ...
    return (
      <nav className="nav">
        ...
        <a href="#" className="nav_link">
          <i className="nav_icon bi-door-closed"></i>
          <span className="nav_name">{t("sign_out")}</span>
        </a>
      </nav>
    );
  };
